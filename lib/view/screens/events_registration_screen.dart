...
class EventsRegistrationScreen extends ConsumerStatefulWidget {
  const EventsRegistrationScreen({
    super.key,
  ...
  @override
  ConsumerState<EventsRegistrationScreen> createState() =>
      _ManageEventsScreenState();
}

class _ManageEventsScreenState extends ConsumerState<EventsRegistrationScreen> {
  late final jwtToken;
  ...
  Future<List<EventList>> getEventsLocality(
    ...
    for (List<double> coordinate in coordinates) {
      String locality = await getLocality(
        lat: coordinate[1],
        lng: coordinate[0],
      );
